...
public class SpaceShipBuilder {
    public static Group build(Vector3D position, int scale) {
        ...
        Box bodyBox = new Box(7.5 * 0.5, 11 * 0.5, 6 * 0.5); // width - height - depth
        ...
        // Nose as a rotated cylinder (cone-like shape) - half size
        Cylinder nose = new Cylinder(3.5 * 0.5, 4.5 * 0.5); // radius, height
        nose.setMaterial(finsMaterial);
        nose.setRotationAxis(Rotate.X_AXIS);
        nose.setRotate(180);   // Lay it forward along X axis
        nose.setTranslateY(-6.5 * 0.5);  // Place on top of the body (half height)

        // Fins
        Box leftFin = new Box(2 * 0.5, 8 * 0.5, 3 * 0.5); // width - height - depth
        leftFin.setTranslateX(-5 * 0.5);
        leftFin.setTranslateY(3 * 0.5);
        leftFin.setMaterial(finsMaterial);
        Box rightFin = new Box(2 * 0.5, 8 * 0.5, 3 * 0.5); // width - height - depth
        rightFin.setTranslateX(5 * 0.5);
        rightFin.setTranslateY(3 * 0.5);
        rightFin.setMaterial(finsMaterial);

        // Glass
        Box glass = new Box(4 * 0.5, 2.3 * 0.5, 0.01 * 0.5); // width - height - depth
        glass.setRotationAxis(Rotate.X_AXIS);
        glass.setRotate(-90);
        glass.setTranslateY(-7.7 * 0.5);
        glass.setTranslateZ(0.6 * 0.5);
        PhongMaterial manMat = BodyDecorations.createMaterialFromImages(Color.LIGHTCYAN, man);
        glass.setMaterial(manMat);

        // Thrust nozzle at the back (half size)
        Cylinder nozzle = new Cylinder(3 * 0.5, 2 * 0.5); // radius - height (a short, wide cylinder)
        nozzle.setRotationAxis(Rotate.X_AXIS);
        nozzle.setTranslateY(6.5 * 0.5);
        nozzle.setMaterial(new PhongMaterial(Color.DARKGRAY));
        nozzle.setMaterial(rocketBodyMaterial);

        // Inner core flame (sharp center) (half size)
        Cylinder innerCore = new Cylinder(1.2 * 0.5, 2 * 0.5);
        innerCore.setRotationAxis(Rotate.X_AXIS);
        innerCore.setTranslateY(9 * 0.5);
        ...
        // Core flame (middle orange) (half size)
        Cylinder coreFlame = new Cylinder(1.5 * 0.5, 2 * 0.5);
        coreFlame.setRotationAxis(Rotate.X_AXIS);
        coreFlame.setTranslateY(9 * 0.5);
        ...
        // Outer flame (soft transparent glow) (half size)
        Cylinder outerFlame = new Cylinder(2.5 * 0.5, 4 * 0.5);
        outerFlame.setRotationAxis(Rotate.X_AXIS);
        outerFlame.setTranslateY(9 * 0.5);
        ...
        // Glow aura (duplicate outer flame with max blur) (half size)
        Cylinder aura = new Cylinder(3.2 * 0.5, 6 * 0.5);
        aura.setRotationAxis(Rotate.X_AXIS);
        aura.setTranslateY(9 * 0.5);
        aura.setRotate(180);
        aura.setScaleX(1.3);
        aura.setScaleZ(1.3);
